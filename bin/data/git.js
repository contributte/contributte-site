...
function sync() {
  _.forEach(CONFIG.resources.repositories.read(), repo => {

    if (!fs.existsSync(`${CONFIG.data.vcsRoot}/${repo.org}`)) {
      fs.mkdirSync(`${CONFIG.data.vcsRoot}/${repo.org}`);
    }

    if (fs.existsSync(`${CONFIG.data.vcsRoot}/${repo.org}/${repo.name}`)) {
      pullRepo(repo);
    } else {
      cloneRepo(repo);
    }
...
function cloneRepo(repo) {
  ...
  const output = spawnSync('git', ['clone', `https://github.com/${repo.org}/${repo.name}.git`], { cwd: `${CONFIG.data.vcsRoot}/${repo.org}` });
...
function pullRepo(repo) {
  ...
  const output = spawnSync('git fetch origin master && git reset --hard origin/master', {
    shell: true,
    cwd: `${CONFIG.data.vcsRoot}/${repo.org}/${repo.name}`
  });
  console.log(`[${repo.org}/${repo.name}]: ${output.status === 0 ? output.stdout : output.stderr}`);
